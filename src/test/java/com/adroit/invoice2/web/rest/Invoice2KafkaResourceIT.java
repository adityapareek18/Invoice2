...
import com.adroit.invoice2.config.KafkaSseConsumer;
import com.adroit.invoice2.config.KafkaSseProducer;
...
class Invoice2KafkaResourceIT {
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/invoice-2-kafka/publish?message=value-produce")).andExpect(status().isOk());
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/invoice-2-kafka/register"))
        ...
        for (int i = 0; i < 100; i++) {
            ...
            if (content.contains("data:value-consume")) {
                restMockMvc.perform(get("/api/invoice-2-kafka/unregister"));
                return;
            }
        }
